...
function Edit_Discover_sweets() {
    const location = useLocation();
    const navigate = useNavigate();
    const { discoverSweetsData } = location.state || {};
    ...
    useEffect(() => {
        setId(discoverSweetsData?._id)
        setName(discoverSweetsData?.name)
        setImage(discoverSweetsData?.image)
        if (discoverSweetsData?.image) {
            setPreviewUrl(`${process.env.REACT_APP_BASE_URL}uploads/${discoverSweetsData?.image}`);
        }
    }, [discoverSweetsData]);
    ...
    const handleSubmit = async () => {
        ...
        if (image && image !== discoverSweetsData?.image) {
            formData.append("image", image);
        }

        try {
            const response = await axios.patch(
                `${process.env.REACT_APP_BASE_URL}api/admin/update_discover_sweets`,
                formData,
                {
                    headers: {
...
export default Edit_Discover_sweets;
